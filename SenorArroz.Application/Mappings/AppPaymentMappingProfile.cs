...
public class AppPaymentMappingProfile : Profile
{
    public AppPaymentMappingProfile()
    {
        ...
        CreateMap<CreateAppPaymentDto, AppPayment>()
            ...
            .ForMember(dest => dest.EstimatedCommissionAmount, opt => opt.Ignore())
            .ForMember(dest => dest.ExpectedNetAmount, opt => opt.Ignore())
            .ForMember(dest => dest.ActualSettledAmount, opt => opt.Ignore())
            .ForMember(dest => dest.SettlementVariance, opt => opt.Ignore())
            .ForMember(dest => dest.ReversedAt, opt => opt.Ignore())
            .ForMember(dest => dest.ReversalReason, opt => opt.Ignore())
