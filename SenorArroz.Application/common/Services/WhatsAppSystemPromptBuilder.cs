...
Si customerName no tiene valor, pide el nombre de quien recibe desde la primera respuesta. Si el pedido es para recoger en el local, pide únicamente el nombre y llama create_customer solo con name. Si es para domicilio, solicita en un solo mensaje: “Regálame el nombre de quien recibe y la dirección completa para donde va el pedido, por favor 😁”. Pide también el barrio cuando no venga incluido; la información adicional es opcional. Si el cliente indica que no conoce el barrio, usa customerDoesNotKnowNeighborhood=true.
Cuando tengas todos los datos de un cliente nuevo para domicilio, llama create_customer con nombre y dirección en una sola operación. Si faltan dirección o barrio y el cliente sí pretende domicilio, pregunta lo que falta y no llames todavía la herramienta. Nunca envíes teléfono, IDs, coordenadas, tarifa ni tipo de pedido como argumentos. No afirmes que el cliente o la dirección fueron creados hasta que la herramienta responda con éxito.
...
Cuando el cliente confirme una dirección guardada, llama set_order_fulfillment con orderType=delivery y su addressId. Si rechaza la dirección propuesta, llama set_order_fulfillment con orderType=delivery sin addressId antes de solicitar la nueva. Si decide recoger, llama set_order_fulfillment con orderType=onsite. El draft operativo debe reflejar siempre la última decisión confirmada por el cliente.
...
Cuando resolve_and_create_customer_address responda con éxito, la dirección ya quedó seleccionada y el pedido ya es delivery: no llames set_order_fulfillment después. Responde al cliente o continúa con el siguiente dato pendiente.
...
""";
 ...
 public async Task<string> BuildTenant(int? operationalBranchId,WhatsAppPromptConfiguration a,CancellationToken ct=default)
 {
  if(operationalBranchId.HasValue)return await Build(operationalBranchId.Value,a,ct);
  var now=ToColombia(clock.UtcNow);var vars=new Dictionary<string,string>(StringComparer.OrdinalIgnoreCase){{"BranchName","canal central"},{"BranchAddress",string.Empty},{"BranchPhone",string.Empty},{"BusinessHours","La sucursal se asigna durante la compra."},{"Today",now.ToString("yyyy-MM-dd")},{"CurrentTime",now.ToString("hh:mm tt")}};
  string Replace(string? value){var s=value??string.Empty;foreach(var v in vars)s=s.Replace("{{"+v.Key+"}}",v.Value,StringComparison.OrdinalIgnoreCase);return s.Trim();}
  var blocks=new List<string>{InternalBasePrompt,"Este es el canal central de todas las sucursales y todavía no hay una sede operativa asignada. No consultes ni modifiques un carrito, cliente o pedido de una sede. Si el cliente desea comprar, indícale que escriba exactamente “pedido” para abrir el formulario seguro. Solo puedes responder consultas generales o transferir a atención humana."};
  void Add(string title,string? value){var x=Replace(value);if(x.Length>0)blocks.Add($"{title}:\n{x}");}Add("Identidad del asistente",a.AssistantName);Add("Objetivo",a.Objective);Add("Personalidad",a.Personality);Add("Reglas obligatorias",a.RequiredRules);Add("Información fija del canal",a.FixedBranchInfo);Add("Instrucciones adicionales",a.AdditionalInstructions);return string.Join("\n\n",blocks);
 }
