...
public interface IWhatsAppOrderDraftSession { Task<WhatsAppOrderDraft?> LoadActiveAsync(int conversationId, CancellationToken cancellationToken = default); Task<WhatsAppOrderDraft> GetOrCreateAsync(AgentToolExecutionContext context, CancellationToken cancellationToken = default); }
public interface IWhatsAppAiContextPlanner { Task<WhatsAppAiContextPlan> PlanAsync(WhatsAppAiContextPlannerInput input, CancellationToken cancellationToken=default); }
...
public interface IWhatsAppProductMatcher{Task<WhatsAppProductMatchResult> MatchAsync(int branchId,string? query,int? servesPeople,int limit=10,CancellationToken cancellationToken=default);}
