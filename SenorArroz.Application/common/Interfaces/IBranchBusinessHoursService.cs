...
public record BranchBusinessHoursEvaluation(bool IsConfigured,bool IsOpen,DateTime? ClosedPeriodStartedAtUtc,DateTime? NextOpeningAtUtc);
public interface IBranchBusinessHoursService{Task<IReadOnlyList<BranchBusinessHourDto>> GetBusinessHours(int branchId,CancellationToken ct=default);Task<string> GetBusinessHoursAsText(int branchId,CancellationToken ct=default);Task<BranchBusinessHoursEvaluation> Evaluate(int branchId,DateTime nowUtc,CancellationToken ct=default);}
