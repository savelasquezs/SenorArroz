...
- La falta de comunicación durante una entrega activa escala a revisión después de dos minutos; en seguimiento liviano escala después de diez minutos. La recuperación conserva el caso pendiente y completa su evidencia.
- El backend clasifica la causa como servicio/app detenido, interrupción de conectividad o no determinable según la telemetría disponible. No molestar no se considera una causa ni genera alerta sin silencio real.
- Las permanencias no esperadas registran domiciliario, hora inicial/final, duración y coordenada central.
- El correo de auditoría diaria identifica al domiciliario e incluye GPS apagado, permiso de ubicación retirado, interrupciones de comunicación y permanencias que requieren revisión, con enlaces de Google Maps cuando existe evidencia. Las ubicaciones offline sincronizadas y la jornada posterior al cierre permanecen disponibles en la app, pero no entran al correo. Antes de construirlo, el cierre de caja procesa los eventos pendientes para no competir con el trabajador periódico.
- Al crear una alerta activa de los mismos tipos incluidos en la auditoría (`gps_disabled`, `location_permission_revoked`, `no_communication` o `unexpected_stay`), el backend envía un aviso FCM inmediato exclusivamente a los dispositivos registrados del domiciliario afectado. El aviso es informativo, advierte sobre una posible falta disciplinaria, permite omitirlo si existía permiso y remite al administrador; no reemplaza la revisión ni registra una decisión disciplinaria.
- El fallo o la ausencia de FCM no revierte ni altera la alerta administrativa. Solo se intenta notificar una alerta cuando se crea activa; reprocesar el seguimiento no vuelve a enviar avisos por la misma alerta.
