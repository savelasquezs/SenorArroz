...
El modelo EF Core contiene 103 entidades: `Tenant` es la única entidad global y las otras 102 implementan `ITenantOwned`. Todas las entidades tenant-owned tienen `tenant_id` obligatorio, índice simple, filtro global, guard de escritura centralizado y RLS forzado.
...
| Inventario | `ExpenseUnitConversion` (`expense_unit_conversion`), `ProductExpenseRequirement` (`product_expense_requirement`), `InventoryBalance` (`inventory_balance`), `InventoryMovement` (`inventory_movement`), `OrderInventoryAllocation` (`order_inventory_allocation`), `InventoryTransfer` (`inventory_transfer`), `InventoryTransferLine` (`inventory_transfer_line`), `InventoryCount` (`inventory_count`), `InventoryCountLine` (`inventory_count_line`) | Tenant | F+G | expense/product/order/branch/transfer/count según jerarquía; `NOT NULL`, FK e índices compuestos por tenant | Compras, pedidos, conteos y transferencias operan bajo tenant y sucursal efectivos; SignalR usa grupo tenant+sucursal |
