...
public class WhatsAppAiContextPlannerTests
{
    ...
    [Fact] public async Task Purchase_intent_does_not_expose_product_card(){var p=await Plan(history:[new("user","Dame un paisa dúo")],incoming:"Dame un paisa dúo");Assert.DoesNotContain("send_product_details",p.AllowedToolNames);Assert.Contains("search_products",p.AllowedToolNames);}
    [Fact] public async Task Information_intent_exposes_product_card(){var p=await Plan(history:[new("user","Qué ingredientes trae el paisa dúo")],incoming:"Qué ingredientes trae el paisa dúo");Assert.Contains("send_product_details",p.AllowedToolNames);}
    ...
    [Fact] public async Task Primary_covered_address_is_in_operational_context_and_suggested(){var customer=new Customer{Id=1,BranchId=1};customer.Addresses.Add(new Address{Id=123,CustomerId=1,AddressText="Cr 59 #27B-387",AdditionalInfo="Boavista torre 3 apto 709",IsPrimary=true,Neighborhood=new Neighborhood{Id=9,BranchId=1,Name="Amazonia-Bello",Active=true}});customer.Addresses.Add(new Address{Id=124,CustomerId=1,AddressText="Otra sucursal",Neighborhood=new Neighborhood{Id=10,BranchId=2,Name="Lejano",Active=true}});var plan=await Plan(draft:Draft(WhatsAppOrderDraftStatus.AwaitingAddress,true),customer:customer);using var json=JsonDocument.Parse(plan.StructuredState!);var addresses=json.RootElement.GetProperty("savedAddresses");Assert.Equal(2,addresses.GetArrayLength());Assert.True(addresses[0].GetProperty("hasCoverage").GetBoolean());Assert.False(addresses[1].GetProperty("hasCoverage").GetBoolean());Assert.Equal(123,json.RootElement.GetProperty("suggestedAddressId").GetInt32());}
    ...
    private static async Task<WhatsAppAiContextPlan> Plan(string strategy="optimized_v1",WhatsAppOrderDraft? draft=null,Customer? customer=null,List<AiChatMessage>? history=null,int maxRuntimeCharacters=6000,string incoming="actual"){using var schema=JsonDocument.Parse("""{"type":"object"}""");var tools=Names.Select(x=>new AiToolDefinition(x,x,schema.RootElement.Clone())).ToList();var planner=new WhatsAppAiContextPlanner(Options.Create(new WhatsAppAiContextOptimizationOptions{MaxRuntimeContextCharacters=maxRuntimeCharacters}),NullLogger<WhatsAppAiContextPlanner>.Instance);return await PlanWith(planner,tools,customer,draft,strategy,history,incoming);}
    private static Task<WhatsAppAiContextPlan> PlanWith(IWhatsAppAiContextPlanner planner,IReadOnlyList<AiToolDefinition> tools,Customer? customer,WhatsAppOrderDraft? draft,string strategy="optimized_v1",List<AiChatMessage>? history=null,string incoming="actual"){history??=[new("user",incoming)];var c=new WhatsAppConversation{Id=1,BranchId=1,PhoneNumber="secret",AttentionMode=WhatsAppAttentionMode.Ai,CustomerId=customer?.Id};var m=new WhatsAppMessage{Id=2,ConversationId=1,TextBody=incoming};return planner.PlanAsync(new(c,m,new Branch{Id=1,Name="Branch"},customer,draft,strategy,20,history,tools,"stable prompt"));}
