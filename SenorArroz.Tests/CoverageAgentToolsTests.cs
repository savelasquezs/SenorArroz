...
public class CoverageAgentToolsTests
{
 ...
 [Fact]public async Task InactiveNeighborhood_DoesNotOfferCoverage(){await using var db=Db();db.Branches.Add(new Branch{Id=1,Name="Centro"});db.Neighborhoods.Add(new Neighborhood{Id=1,BranchId=1,Name="Santander",DeliveryFee=4000,Active=false});await db.SaveChangesAsync();var r=await new RegisteredNeighborhoodResolver(db).Resolve("Santander",1,default);Assert.False(r.Matched);Assert.False(r.RequiresConfirmation);}
