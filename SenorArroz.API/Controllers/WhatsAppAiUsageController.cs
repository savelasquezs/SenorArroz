    ...
    IBranchContext branchContext) : ControllerBase
{
    ...
    public async Task<ActionResult<ApiResponse<WhatsAppAiUsageDto>>> Get(int? branchId, DateOnly? fromDate, DateOnly? toDate, string? provider, string? model, string? contextStrategy, CancellationToken ct)
    {
        ...
        var normalizedStrategy=contextStrategy?.Trim().ToLower();if(!string.IsNullOrEmpty(normalizedStrategy))q=q.Where(x=>x.ContextStrategy==normalizedStrategy);
        ...
        var a = await q.GroupBy(_ => 1).Select(g => new { Input=g.Sum(x=>(long)(x.InputTokens??0)), Cached=g.Sum(x=>(long)(x.CachedInputTokens??0)), Output=g.Sum(x=>(long)(x.OutputTokens??0)), Thinking=g.Sum(x=>(long)(x.ThinkingTokens??0)), Billable=g.Sum(x=>(long)(x.BillableOutputTokens??x.OutputTokens??0)), Cost=g.Sum(x=>x.EstimatedCostUsd??0), Unpriced=g.Count(x=>x.EstimatedCostUsd==null), Errors=g.Count(x=>!x.Success), Duration=g.Average(x=>(double?)x.DurationMs)??0, Tools=g.Sum(x=>x.ToolCallCount),ContextMessages=g.Average(x=>(double?)(x.ContextMessageCount??0))??0,ToolDefinitions=g.Average(x=>(double?)(x.ToolDefinitionCount??0))??0,Fallbacks=g.Count(x=>x.ContextPlannerFallback)}).FirstOrDefaultAsync(ct);
        ...
        var breakdown = await q.GroupBy(x=>new{x.Provider,x.Model,x.ContextStrategy}).Select(g=>new WhatsAppAiUsageBreakdownDto(g.Key.Provider,g.Key.Model,g.Key.ContextStrategy,g.Count(),g.Select(x=>x.IncomingMessageId).Distinct().Count(),g.Sum(x=>(long)(x.InputTokens??0)),g.Sum(x=>(long)(x.CachedInputTokens??0)),g.Sum(x=>(long)(x.OutputTokens??0)),g.Sum(x=>(long)(x.ThinkingTokens??0)),g.Sum(x=>(long)(x.BillableOutputTokens??x.OutputTokens??0)),g.Sum(x=>x.EstimatedCostUsd??0),g.Count(x=>x.EstimatedCostUsd==null),g.Average(x=>(double?)x.DurationMs)??0,(double)g.Count(x=>!x.Success)/g.Count(),g.Average(x=>(double?)(x.ContextMessageCount??0))??0,g.Average(x=>(double?)(x.ToolDefinitionCount??0))??0,(double)g.Count(x=>x.ContextPlannerFallback)/g.Count())).ToListAsync(ct);
