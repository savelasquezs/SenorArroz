...
public class WhatsAppAiUsageController(IApplicationDbContext db, ICurrentUser currentUser) : ControllerBase
{
    ...
    public async Task<ActionResult<ApiResponse<WhatsAppAiUsageDto>>> Get(int? branchId, DateOnly? fromDate, DateOnly? toDate, string? provider, string? model, CancellationToken ct)
    {
        ...
        var a = await q.GroupBy(_ => 1).Select(g => new { Input=g.Sum(x=>(long)(x.InputTokens??0)), Cached=g.Sum(x=>(long)(x.CachedInputTokens??0)), Output=g.Sum(x=>(long)(x.OutputTokens??0)), Thinking=g.Sum(x=>(long)(x.ThinkingTokens??0)), Billable=g.Sum(x=>(long)(x.BillableOutputTokens??x.OutputTokens??0)), Cost=g.Sum(x=>x.EstimatedCostUsd??0), Unpriced=g.Count(x=>x.EstimatedCostUsd==null), Errors=g.Count(x=>!x.Success), Duration=g.Average(x=>(double?)(x.DurationMs??0))??0, Tools=g.Sum(x=>x.ToolCallCount)}).FirstOrDefaultAsync(ct);
        var p95 = total == 0 ? 0 : await q.Where(x=>x.DurationMs.HasValue).OrderBy(x=>x.DurationMs).Select(x=>x.DurationMs!.Value).Skip(Math.Max(0,(int)Math.Ceiling(total*.95)-1)).FirstOrDefaultAsync(ct);
